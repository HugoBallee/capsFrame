    ...
    for (p=y; p<y+height; ++p) {
      if (colorsIn.length > 0) {
        ...
        if (indexColor >= 0) {
          ...
          colorsIn.push({color:colors[i][j], count:1});
        }
      } else {
        colorsIn.push({color:colors[i][j], count:1});
      }
...
function addColors(colorA, colorB) {
  return {r:colorA.r+colorB.r, g:colorA.g+colorB.g, b:colorA.b+colorB.b};
}
...
function divideColor(color, div) {
  if (div <= 0)
    ...
    return {r:(color.r/div), g:(color.g/div), b:(color.b/div)};
}
...
function pictureData2ColorArray(pictureData) {
  ...
  for (i=0; i<pictureData.length; i+=4) {
    var color = {r:pictureData[i], g:pictureData[i+1], b:pictureData[i+2]};
    colors.push(color);
  }
  return colors;
}
